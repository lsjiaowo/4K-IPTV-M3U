...
# ================= 配置区域 =================
MULTICAST_SOURCE_URL = "https://iptv.cqshushu.com/index.php"
...
def fetch_region_rows_from_html(session, province, max_pages=30):
    ...
    for page_num in range(1, max_pages + 1):
        # 🚨 核心修复：抛弃失效的 province 字段，直接使用全站关键字搜索参数 q
        url = f"{MULTICAST_SOURCE_URL}?t=multicast&q={quote(province)}&page={page_num}"
        ...
        # 翻页结束条件：如果当前页没有抓到任何节点，说明到底了
        if added == 0: break

    print(f"[*] [{province}] 全部分页合计抓取到 {len(all_rows)} 个有效服务器。")
    return all_rows
...
def extract_channels_for_token(session, p_token):
    """
    黑盒探测：尝试枚举常见的详情页 API 和路由
    """
    endpoints = [
        f"https://iptv.cqshushu.com/detail.php?p={p_token}&type=multicast",
    ...
    for url in endpoints:
        try:
            ...
            if lines:
                return list(dict.fromkeys(lines))  # 去重返回
        except Exception:
            continue
...
def fetch_channel_lines_by_province(province: str, max_per_carrier: int = 10, max_pages: int = 30, max_age_hours: int = 72):
    ...
    for picked in selected_rows:
        ...
        if lines:
            ...
            print(f"[-] 成功锁定优质节点: {picked['host']} ({group_title})，但探测详情页路由失败，被站长 JS 拦截。")
